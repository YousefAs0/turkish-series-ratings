...
def run():
    ...
    with open(latest_file, 'r', encoding='utf-8') as f: jd = json.load(f)

    auth = tweepy.OAuth1UserHandler(X_K, X_S, X_T, X_TS)
    api_v1 = tweepy.API(auth)
    client_v2 = tweepy.Client(consumer_key=X_K, consumer_secret=X_S, access_token=X_T, access_token_secret=X_TS)
    ...
    for s in shows:
        ...
        try:
            ...
            print(f"Posted with Image: {s['name']}")
            os.remove(tmp)
        except Exception as e:
            # إذا فشل بسبب الدفع (402)، انشر النص فقط
            if "402" in str(e) or "Payment" in str(e):
                client_v2.create_tweet(text=txt)
                print(f"Posted Text Only (Subscription Required for Images): {s['name']}")
            else:
